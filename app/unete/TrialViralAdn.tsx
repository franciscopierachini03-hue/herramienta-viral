...
// Página de pago PARALELA para comunidades — componente COMPARTIDO.
// La usan /unete (genérica, canal 'comunidad') y /adama (comunidad ADAMA de
// Regina, canal 'adama'). Comunidad nueva = crear app/<nombre>/page.tsx que
// renderice <TrialViralAdn canalDefault="<nombre>" comunidad="<NOMBRE>" />.
//
...
function canalActual(porDefecto: string): string {
  try {
    const c = new URLSearchParams(window.location.search).get('canal') || porDefecto;
    return c.toLowerCase().slice(0, 40);
  } catch { return porDefecto; }
}

export default function TrialViralAdn({ canalDefault, comunidad }: { canalDefault: string; comunidad?: string }) {
  ...
  async function empezar() {
    ...
    try {
      const res = await fetch('/api/checkout', {
        ...
        body: JSON.stringify({ producto: 'viraladn', ciclo: 'monthly', trial: true, canal: canalActual(canalDefault), origen: 'unete', cuenta: 'elevation' }),
      });
    ...
    <main className="min-h-screen text-white" style={{ background: 'radial-gradient(ellipse 100% 40% at 50% 0%, #1a0a2e 0%, #080808 55%)' }}>
      ...
      <section className="text-center px-6 pt-8 pb-10 max-w-2xl mx-auto">
        <span className="inline-block text-xs font-extrabold tracking-widest uppercase px-4 py-2 rounded-full mb-5"
          ...
          {comunidad ? `🎁 Comunidad ${comunidad} · 7 días gratis` : '🎁 7 días gratis · sin compromiso'}
        </span>
