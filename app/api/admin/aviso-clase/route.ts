...
import { CLASE } from '@/app/comunidad/clase-config';
...
function emailHtml(hora: string, fechaTxt: string): { subject: string; html: string } {
...
<div style="max-width:520px;margin:0 auto;background:#101018;border:1px solid #23232e;border-radius:18px;padding:28px;">
  ...
  <p style="margin:0 0 16px;font-size:15px;color:#c8c8d4;line-height:1.55;">
    <b style="color:#fff;">${fechaTxt}</b> la clase <b style="color:#fff;">${CLASE.nombre}</b> comienza a las
  ...
  <div style="background:#0b0b10;border:1px solid #23232e;border-radius:14px;padding:16px;margin:0 0 18px;">
    <p style="margin:0 0 6px;font-size:13px;color:#9a9aa6;">📍 Sala: <b style="color:#fff;">${CLASE.sala}</b></p>
    <p style="margin:0 0 6px;font-size:13px;color:#9a9aa6;">ID: <b style="color:#fff;font-family:monospace;">${CLASE.zoomId}</b> · Código: <b style="color:#fff;font-family:monospace;">${CLASE.zoomCodigo}</b></p>
  </div>
  <a href="${CLASE.zoomUrl}" style="display:block;text-align:center;background:linear-gradient(90deg,#7c3aed,#ec4899);color:#fff;font-weight:800;font-size:16px;padding:14px;border-radius:12px;text-decoration:none;">
...
export async function GET(req: NextRequest) {
  ...
  const { subject, html } = emailHtml(hora, fechaTxt);
  ...
  if (test) {
    ...
    return Response.json({ modo: 'test', para: OWNER, ...r, aviso: fechaTxt + ' · ' + hora });
  }

  if (!mandar) {
    return Response.json({ modo: 'dry', recibirian: emails.length, aviso: fechaTxt + ' · ' + hora, siguiente: 'agregá &test=1 para verlo vos, o &enviar=1 para mandarlo a todos' });
  }
  ...
  console.log(`[aviso-clase] enviado a ${r.enviados} miembros (${fechaTxt} · ${hora})`);
  return Response.json({ modo: 'enviado', destinatarios: emails.length, ...r, aviso: fechaTxt + ' · ' + hora });
}
